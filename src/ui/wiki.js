...
export class WikiPage {
  ...
  _build() {
    this.root.innerHTML = `<main class="wiki-shell">
      ...
      <div><div class="wiki-kicker">${COPY.subtitle}</div><h1>${COPY.title}</h1><p class="wiki-summary">${COPY.summary}</p><p class="wiki-input-note">${COPY.statusNote}</p><p class="wiki-input-note">${COPY.boundaryNote}</p><p class="wiki-input-note">${COPY.footprintNote}</p><p class="wiki-input-note">${COPY.attackNote}</p><p class="wiki-input-note">${COPY.combatSequenceNote}</p><p class="wiki-input-note">${COPY.spriteNote}</p><p class="wiki-input-note">${COPY.inventoryNote}</p><p class="wiki-lint-note">${LINT_NOTE}</p><p class="wiki-input-note">${INPUT_NOTE}</p></div>
      </header>
