...
const COPY = Object.freeze({
  ...
  boundaryNote: '\u623f\u95f4\u56f4\u5899\u4e0d\u56e0\u89d2\u8272\u9760\u8fd1\u6216\u79bb\u5f00\u800c\u9690\u85cf\uff1b\u5357\u9762\u53ea\u56fa\u5b9a\u4e0d\u751f\u6210\u4e2d\u95f4\u67f1\u5b50\uff0c\u4e24\u7aef\u67f1\u5b50\u4fdd\u7559\u3002\u654c\u4eba\u7ffb\u724c\u4f7f\u7528\u4e2d\u6027\u5361\u80cc\uff0c\u4e14\u4e34\u65f6\u724c\u53d7\u5357\u5899\u906e\u6321\u3002',
  ...
  inventoryNote: 'Inventory is touch-only: hold an occupied item for 300 ms, move more than 18 px to drag, and use a second touch for clockwise 90-degree rotation. The floating footprint snaps by its center with a half-cell drop tolerance. Green, yellow, and red previews mean accept, replace-to-staging, and illegal. The red discard zone and blue free-form staging canvas exactly fill the whole area above the backpack in a 25/75 split; staged items preserve backpack-cell size and seek a zero-overlap automatic position, while the detail panel remains above both zones for inspection.',
  implemented: '\u5df2\u5b9e\u88c5',
  back: '\u8fd4\u56de\u5730\u7262',
  enemies: '\u654c\u4eba',
  traps: '\u9677\u9631',
...
export class WikiPage {
  ...
  _build() {
    this.root.innerHTML = `<main class="wiki-shell">
      ...
      <div><div class="wiki-kicker">${COPY.subtitle}</div><h1>${COPY.title}</h1><p class="wiki-summary">${COPY.summary}</p><p class="wiki-input-note">${COPY.statusNote}</p><p class="wiki-input-note">${COPY.boundaryNote}</p><p class="wiki-input-note">${COPY.footprintNote}</p><p class="wiki-input-note">${COPY.attackNote}</p><p class="wiki-input-note">${COPY.combatSequenceNote}</p><p class="wiki-input-note">${COPY.pickupSequenceNote}</p><p class="wiki-input-note">${COPY.spriteNote}</p><p class="wiki-input-note">${COPY.inventoryNote}</p><p class="wiki-lint-note">${LINT_NOTE}</p><p class="wiki-input-note">${INPUT_NOTE}</p></div>
      </header>
