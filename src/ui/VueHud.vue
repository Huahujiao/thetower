...
<template>
  <div class="vue-hud-root" @click="onClick" @pointerdown="onPointerDown" @pointermove="onPointerMove" @pointerup="onPointerUp" @pointercancel="onPointerUp" @contextmenu="onContextMenu">
    ...
    <div class="relic-choice" :class="{ show: initialRelicOpen }"><div class="relic-choice-title">{{ LABELS.initialRelic }}</div><div class="relic-choice-row"><button v-for="relic in initialRelics" :key="relic.id" class="relic-choice-card" :data-relic-choice="relic.id" @click.stop="selectInitialRelic(relic.id)"><span class="relic-name">{{ relic.name }}</span><span class="relic-desc">{{ relic.description }}</span></button></div></div>
    ...
    <div class="hud-over" :class="{ show: state.gameOver, win: state.win, lose: !state.win }"><h1>{{ state.win ? LABELS.win : LABELS.lose }}</h1><p>{{ state.win ? LABELS.winMessage : LABELS.loseMessage }}</p><button data-action="restart" @click.stop="restartGame">{{ LABELS.restart }}</button></div>
  </div>
</template>
