...
`GameRun` 维护地牢、玩家生命／护甲／体力、5×9 背包、角色成长、圣遗物、商人、奖励、状态效果、敌人状态和日志。圣遗物以 `type: 'relic'` 的 1×1 背包物品保存；`RelicCollection` 是由背包物品同步出的效果索引。每个非攻击全局回合回复 1 点体力；攻击推进攻击计数和全局回合并消耗 2–5 点体力。接近路径中的每格移动也是独立的非攻击回合。

回合计数只有 `attackCount` 和 `globalTurn`；`turn` 是全局回合的兼容别名。背包整理、丢弃、奖励选择、升级选择和商人面板操作不推进计数。运行时不存在左右手、装备栏、行动计数、武器耐久、磨刀石、最后一击、武器损毁或拦截机制。背包中的圣遗物超过 5 件时，`RelicEngine` 不提供任何圣遗物效果；减少到 5 件或以下后恢复。
...
当前存档版本为 **21**。版本号不匹配、结构无效或玩家位置无效时会删除存档并创建新局；存档同时保留 `turn` 作为全局回合兼容字段。旧的独立圣遗物收藏存档不迁移。
