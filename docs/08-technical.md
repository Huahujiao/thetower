...
`GameScene` places movement, flip, and attack events in one FIFO animation queue. Only one attack animation can be active at a time, and the queue remains a rebuild barrier until the animation completes. The temporary attack duration is 0.5 seconds. The player pose raises both arms and briefly brings the feet together; the enemy pose lifts the circular head and sways the triangular body twice before settling. A player attack carries an immutable post-hit target-status snapshot; when that player animation completes, only the target's Three.js status overlay is refreshed before the next queued enemy animation starts. The renderer restores the original texture and transform after each action, so combat state and hit resolution remain unchanged. Attack pose canvases are redrawn from an identity 2D transform on every frame; the temporary redraw scale is applied exactly once, preventing the character texture from growing and being cropped to one corner.
