...
Animation tracks address `joint:<id>` or `part:<id>` targets and store XYZ offsets, XYZ Euler rotations, XYZ scale, and opacity. The fixed slots remain `idle`, `attack`, `hit`, `death`, and `move`. `ShadowPuppetStage.vue` is a shared Three.js canvas. Front mode shows edit guides; 3D mode suppresses guides, bones, and target editing, sets an elevated orbit camera, and draws a 5×5 ground patch from the exact four floor assets and placement hash used by the game board. Floor height follows the lowest rest-pose joint plus serialized `stage.floorOffset` (8 by default; 44 for the hovering moth). The separate `/animepreview` route remains available for future multi-enemy preview work, but its editor header link is removed. Neither tool route instantiates `GameRun`.

The roster retains its `project-v3` and `roster-v3` local-storage keys for compatibility, while normalized projects and rosters use schema version 4. If no v3-key data exists, v2 data is read and migrated. `shadow-examples.js` installs three editable monster rigs once: a bell-bearing humanoid, a six-legged tide spider, and a lantern moth. Their root yaw is +45° after conversion to positive-Z-front coordinates. Existing rosters with `examplePackVersion: 1` receive a one-time version-2 example repair: match example structure, append only missing original parts whose binding targets still exist, and correct an unchanged -45° root yaw to +45° in the new convention. Existing part parameters and unrelated characters are preserved. The roster marker then prevents deleted parts from reappearing again. The active project is mirrored to the project key for `/animepreview` and cross-tab updates.

`ShadowHistory` keeps bounded in-memory snapshots of the full roster for undo. Vue's deep roster watcher captures all persisted mutations; pointer drags and focused form edits are grouped into single steps. Undo restores a normalized snapshot and saves it immediately. Per-character hidden-part IDs are editor-only state and never serialized. `npm run check:animation` checks rig migration/kinematics, example integrity, one-time repair, and undo grouping.
...
The animation stage maintains separate cameras over one shared rig evaluation: an orthographic camera for flat editing and a 48-degree perspective camera for 3D preview. Both begin on positive Z, making greater Z closer to the viewer. The flat view now writes part depth so front parts occlude rear parts instead of relying solely on layer order; layer order still resolves equal-depth overlaps. The preview uses Three.js `OrbitControls` on the canvas (one-finger orbit, two-finger dolly/rotate), with panning disabled. Its character group is scaled and centered from the rest-pose joint/part bounds to fit within one 150-unit floor tile; this is a presentation transform only and is not saved to the rig. The floor stays beneath the rest-pose low point. Project schema v4 reflects stored v3 Z positions, X/Y rotations, and animation offsets/rotations exactly once; normalizing v4 data is idempotent.
...
The animation panel keeps playback, duration, loop, a scrubber, and all-track Record/Delete controls. Its transform editor switches between XYZ displacement, Euler rotation, and scale; opacity remains available. Undo captures keyframe edits and reset as roster snapshots. Reset clears only the active animation tracks after confirmation.
...
`VueHud.vue` derives `statusEntries` from the event-invalidated `GameRun` view. Poison and burning contribute explicit remaining-turn entries. Other entries come only from active non-relic `player.itemState.buffs`, covering temporary consumable, combat, and talent effects such as rage wine; relic counters, persistent equipment/talent conditions, defensive counters, and spatial build checks do not enter the tray. `status-icons.js` maps poison, burning, rage wine, generic attack gain, and energy discount to alpha-transparent 256 px small assets; mixed attack/discount buffs prioritize the attack icon while the badge and long-press detail preserve the actual effect. The matching 512 px variants remain available and high-resolution sources stay under `src/assets/status/backup/source/`, but the 28 px tray loads only the small files. A tray entry owns its touch lifecycle and starts the shared 150 ms hold timer; it uses the existing `GameRun` detail event and `detailPanelVisible`, then closes on release. The tray is a canvas-container overlay anchored to the lower-left of the Three.js scene, flowing from left to right and therefore aligning with the actual scene bounds rather than an estimated HUD height.
