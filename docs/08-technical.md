...
The version-3 roster uses `project-v3` and `roster-v3` local-storage keys, reading v2 data once if no v3 data exists. `shadow-examples.js` installs three editable monster rigs once: a bell-bearing humanoid, a six-legged tide spider, and a lantern moth. Their root yaw is now −45°. Existing rosters with `examplePackVersion: 1` receive a one-time version-2 repair: match example structure, append only missing original parts whose binding targets still exist, and flip unchanged +45° root yaw to −45°. Existing part parameters and unrelated characters are preserved. The roster marker then prevents deleted parts from reappearing again. The active project is mirrored to the v3 project key for `/animepreview` and cross-tab updates.
