...
The reviewed sprite set currently includes twelve weapon/defense sprites plus six single-cell item sprites: rust sword, bone knife, ember spear, root axe, mountain maul, silver guard, ember axe, tide blade, thorn spear, wood bow, ash bow, wooden ward shield, health potion, iron powder, energy potion, cleanse, rage wine, and teleport. The HUD uses these sprites without labels or procedural cell fills when their item ids are present; other items retain distinct cool dark fills by type and attribute, while the wiki's balance cards remain driven by the item catalog. Sprite URLs are bundled as production assets, and clicking a sprite selects its first occupied cell so the starter weapon remains usable even when transparent pixels cover the cell. Browser context menus are disabled within the backpack so long-press inspection does not leave the game UI.
