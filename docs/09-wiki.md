...
`/animeedit` is the 3D enemy skeleton animation editor. It keeps an automatically saved `localStorage` roster with three editable monster examples and a blank character. The examples now use −45° root yaw; a one-time repair restores missing original sample parts without overwriting existing part edits. The flat editing camera now looks along each character's rest-pose forward direction. The 3D toggle is a perspective-camera single-enemy preview: the edit grid and skeleton disappear, and the same floor tile artwork and distribution as the game board appear beneath the monster. The character's resting footprint fits within one floor tile. Three.js OrbitControls handles one-finger orbit and two-finger zoom. The separate `/animepreview` route remains for possible future group previews but is no longer linked from the editor header. Existing v2 characters still migrate to Z=0.
