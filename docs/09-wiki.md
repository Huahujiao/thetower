...
`/animeedit` is the 3D enemy skeleton animation editor. It keeps an automatically saved `localStorage` roster selected through a dropdown: characters can be added, renamed, duplicated, deleted, and switched. There are no manual save, import, export, or example controls. The editor starts with one blank character, and deleting the final character creates another blank replacement. Version-3 storage reads and migrates existing v2 characters to the Z=0 plane. `/animepreview` is the read-only player for the current character. Both routes share a standalone Three.js stage, separate from gameplay.
