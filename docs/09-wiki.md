...
`/animeedit` is the 3D enemy skeleton animation editor. It keeps an automatically saved `localStorage` roster with three editable monster examples and a blank character. The examples now use +45° root yaw in the positive-Z-front convention; a one-time repair restores missing original sample parts without overwriting existing part edits. Flat editing and 3D preview use the same rig and animation data, with both cameras on positive Z so larger Z is closer to the viewer. The 3D toggle is a perspective-camera single-enemy preview: the edit grid and skeleton disappear, and the same floor tile artwork and distribution as the game board appear beneath the monster. The character's resting footprint fits within one floor tile. Three.js OrbitControls handles one-finger orbit and two-finger zoom. The separate `/animepreview` route remains for possible future group previews but is no longer linked from the editor header. Existing v3 local characters migrate their depth and X/Y rotations exactly once; v2 characters still migrate from Z=0.
...
The header has Back, character selection/management, and Undo. Every persisted edit is undoable from a bounded in-memory roster history; one drag or focused input session forms one step. Parts mode has a per-part temporary Hide/Show eye control that affects only the editor view, not saved data or the standalone preview route. The portrait layout retains a fixed 204px lower panel. Front mode supports panning and pinch zoom; its Grid checkbox hides geometry and guides together. In 3D mode, drag orbits around the single monster and the floor replaces the edit grid.
...
Animation mode retains playback, a timeline, and all-track Record/Delete actions. XYZ displacement, rotation, and scale are selected through compact sub-tabs; opacity remains available. The lower panel is fixed at 204px. Reset only clears the active action's keys, and the change can be undone.
...
The supported interaction target is portrait mobile touch only. Inventory long press is bound directly to each occupied Vue-rendered cell with `touchstart`, `touchmove`, `touchend`, and `touchcancel`; the Three.js board detail hold uses the same 150 ms threshold. Desktop mouse, keyboard, stylus, and landscape layouts are outside the product contract.

After 150 ms, the corresponding detail panel is populated and shown through `detailPanelVisible`; releasing the touch closes it, while movement or cancellation aborts the pending timer.

## Scene status tray

The main HUD no longer has a separate build-status text row. The experience strip sits directly below the top HUD, and the released space belongs to the Three.js board. Active statuses render inside the board's lower-left edge, flowing from left to right. Poison, burning, rage wine, generic attack-up, and reduced-energy-cost effects use dedicated transparent pictograms, while the corner badge keeps the remaining turns or effect value. Holding a cell uses the existing 150 ms detail gesture and releasing closes the detail overlay. The wiki header records this interaction alongside the current content reference.
