...
| 圣遗物 | `src/game/data/relics.js` 的 `RELIC_DEFS`，当前 6 件 |
| 天赋 | `src/game/data/progression.js` 的 `TALENT_DEFS`，当前 20 个节点 |
...
`/animeedit` is the 3D enemy skeleton animation editor. It keeps an automatically saved `localStorage` roster selected through a dropdown: characters can be added, renamed, duplicated, deleted, and switched. There are no manual save or import/export controls. Three editable monster examples—bell pilgrim, tide-eyed spider, and lantern moth—are installed once without overwriting existing characters. Each has all five action tracks and real Z-axis depth motion, with a +45° root yaw facing left-front. A blank character remains available for scratch work; deleting an example does not cause it to return. Version-3 storage reads and migrates existing v2 characters to the Z=0 plane. `/animepreview` is the read-only player for the current character. Both routes share a standalone Three.js stage, separate from gameplay.

The authoring order is joint -> directed bone link -> rendered part -> animation. Joints, parts, and animation tracks expose XYZ position and rotation, while animation tracks additionally expose XYZ scale and opacity. Geometric parts remain flat shapes that can be placed and rotated in 3D, with free, joint, or bone attachment. The stage supports one-finger pan, two-finger zoom, a 3D orbit toggle, and front-view reset; preview allows viewing but not editing. Texture fields remain reserved for a later texture mode. Character names never receive a “shadow puppet” suffix.

The character dropdown plus Rename, New, Duplicate, and Delete icons sit in the first row between Back and Preview. Rename opens a name dialog; Delete asks for confirmation. In portrait mobile layout, header and mode menu occupy 8% and 6% of viewport height, and the workspace fills the remaining height. The page itself never scrolls. The bottom panel is fixed at 171px in all three tabs, based on the five visible rows when a part is selected; switching tabs or selecting a part never changes the stage height. Extra content, if any, scrolls within the panel. In Parts mode, Rename, Duplicate, and Delete are icon buttons in that order. Attachment, target, and optional Position share one row; width, height, and rotation share another; layer, fill, and stroke share a third. All field labels have the same 20px width and sit beside their controls, aligning inputs across rows. Dragging blank canvas with one finger pans the view; a two finger pinch zooms around its midpoint. Dragging a joint or part still edits its position, and a short blank canvas tap still invokes the active creation tool. View gestures do not edit or save rig coordinates. The stage remains visible in Skeleton, Parts, and Animation and uses a flat blue-gray background. Disabling Grid hides both rendered geometry parts and the background guide grid, leaving only joints and bone links. Joint markers render above every other stage layer.

In Skeleton Select mode, the instructional status line is absent; other tool and error messages remain. A selected joint shows its name on a full-width first row, with X, Y, and rotation across a second row. A selected bone shows its name and connection on separate full-width rows. The connection never wraps and only truncates with an ellipsis if its names are unusually long.

In Animation mode, the playback row gives Play, Rewind, the time readout, and Loop fixed no-wrap widths. The Duration input takes the remaining width, adapting to the phone screen without forcing the other controls onto multiple lines. Record Frame and Delete Frame sit to the right of the timeline and operate on all joints and parts at the current time, not just the selected object. Timeline markers combine key times from every track; bone links follow joints and have no separate animation keys. The six transform fields use two rows of three, with labels inline beside their inputs. The separate selected-target caption is omitted so all five tool rows fit the 171px panel without scrolling; canvas selection remains highlighted. Reset at the end of the animation tabs clears the selected action's joint and part keyframes after confirmation, stops playback, and returns to time zero and the base rig pose. It does not alter other actions, the rest rig, duration, or loop setting.

The animation timeline uses an explicit 16px slider thumb with no native input margin. The visible progress track and frame markers start and end 8px inside the input, matching the thumb-center travel range; a marker, the progress color, and the thumb at the same time now share one horizontal center.
