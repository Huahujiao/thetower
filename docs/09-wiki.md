...
`/animeedit` is the flat 2D enemy animation editor. It keeps an automatically saved `localStorage` roster selected through a dropdown: characters can be added, renamed, duplicated, deleted, and switched. There are no manual save, import, export, or example controls. The editor starts with one completely blank character, and deleting the final character creates another blank replacement. Fresh v2 storage keys prevent the retired sample roster from repopulating the page. `/animepreview` is the read-only player for the current character. Neither route initializes Three.js.

The authoring order is joint -> directed bone link -> rendered part -> animation. Skeleton mode creates independent joints and links two tapped joints; unlinked joints are valid. Parts mode creates geometric shapes and leaves them free or binds them to a joint or a point along a bone. Animation mode writes transform and opacity tracks for a selected joint or part in Idle, Attack, Hit, Death, or Move. Joints, bones, and parts are separate data layers. Texture fields are reserved in the data model for a later texture mode, but the current editor exposes geometry only. Character names never receive a “shadow puppet” suffix.
