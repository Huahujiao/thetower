...
The reviewed sprite set currently includes a 1x2 rust sword, 1x1 bone knife, 1x3 ember spear, an L-footprint root axe, a cross-footprint mountain maul, and a 1x2 wooden ward shield. The HUD uses these sprites without labels or procedural cell fills when their item ids are present; other items retain distinct cool dark fills by type and attribute, while the wiki's balance cards remain driven by the item catalog. Sprite URLs are bundled as production assets, and clicking a sprite selects its first occupied cell so the starter weapon remains usable even when transparent pixels cover the cell. Browser context menus are disabled within the backpack so long-press inspection does not leave the game UI.
