...
Attack presentation stays within the original character/card plane size. Each frame redraws its temporary pose from an identity canvas transform, so the 0.5-second animation cannot accumulate bitmap scaling or crop the character to a corner. After a player pose completes, the struck enemy health bar updates before any queued enemy pose starts.
