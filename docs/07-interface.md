...
`/animeedit` 是独立的三维敌人骨架动画编辑器。角色库通过下拉框切换角色，并可新增、重命名、复制和删除；删除最后一个角色后会立即补充一个空白角色。每个角色的关节、骨骼线、几何部件及五套动作都自动保存到 `localStorage`，页面不提供手动保存、导入或导出。编辑器不内置或自动载入示例角色，首次进入只创建一个空白角色。角色名称只使用本名，不附加“皮影”后缀。旧版二维角色自动迁移到 Z=0 平面，保留原有关键帧。
